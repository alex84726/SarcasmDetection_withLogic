...
class HiddenLayer(object):
    ...
    def __init__(self, rng, input, n_in, n_out, activation, W=None, b=None):
        ...
        if W is None:
            ...
            For more reference on its use:
                http://deeplearning.net/software/theano/tutorial/aliasing.html
            """
        if b is None:
            b_values = np.zeros((n_out,), dtype=theano.config.floatX)
            # make b a shared variable:
...
#---------------------------------------------------------------------------------
class DropoutHiddenLayer(HiddenLayer): # class (derived) DropoutHiddenLayer inherits from class HiddenLayer
    def __init__(self, rng, input, n_in, n_out,  activation, dropout_rate, W=None, b=None):
        super(DropoutHiddenLayer, self).__init__(
                 rng=rng, input=input, n_in=n_in, n_out=n_out, W=W, b=b,
                 activation=activation)

        """ # super(,self).method: inherits method from base class
        class C(B):
            def method(self, arg):
                super(C, self).method(arg)
        """

        self.output = self._dropout_from_layer(rng, self.output, p=dropout_rate)
    # (internal use only): dropout + recast output as theano.config.floatX
    def _dropout_from_layer(self, rng, layer, p):
...
class MLPDropout(object):
    ...
    def __init__(self,rng,input,layer_sizes,dropout_rates,activations):
        ...
        self.weight_matrix_sizes = zip(layer_sizes, layer_sizes[1:]) # list of (matrix sizes = (layer_sizes[i],layer_sizes[i+1]))
        self.layers = [] # list of each layer's outputs
        self.dropout_layers = [] # list of each  dropout layer's ouputs
        self.activations = activations
        next_layer_input = input
        #first_layer = True
        # dropout the input
        next_dropout_layer_input = self._dropout_from_layer(rng, input, p=dropout_rates[0])
        layer_counter = 0
        for n_in, n_out in self.weight_matrix_sizes[:-1]:
            next_dropout_layer = DropoutHiddenLayer(rng=rng, # output of DropoutHiddenLayer
