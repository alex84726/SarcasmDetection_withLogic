...
def train_conv_net(datasets,
    ...
    parameters = [("image shape", img_h,
                   img_w), ("filter shape", filter_shapes), ("hidden_units",
                                                             hidden_units),
                  ("dropout", dropout_rate), ("batch_size", batch_size),
                  ("non_static", non_static), ("learn_decay",
                                               lr_decay), ("conv_non_linear",
                                                           conv_non_linear),
                  ("non_static", non_static), ("sqr_norm_lim",
                                               sqr_norm_lim), ("shuffle_batch",
                                                               shuffle_batch)]
    print parameters
    ...
    train_set_x, train_set_y = shared_dataset((train_set[:, :img_h],
                                               train_set[:, -1]))
    ...
    test_model_all = theano.function(
        [x, y], test_error, allow_input_downcast=True)
...
def sgd_updates_adadelta(params,
                         cost,
                         rho=0.95,
                         epsilon=1e-6,
                         norm_lim=9,
    ...
    for param, gp in zip(params, gparams):
        ...
        if (param.get_value(
                borrow=True).ndim == 2) and (param.name != 'Words'):
