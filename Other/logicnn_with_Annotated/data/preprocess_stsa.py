...
if __name__=="__main__":    
    stsa_path = sys.argv[1] #original data
    w2v_file = sys.argv[2] #Google word to vector
    ...
    max_l = np.max(pd.DataFrame(revs)["num_words"]) # max length of sentence)
    print "data loaded!"
    ...
    # w2v is a dict which given a vocabulary in vocab, then it will return a
    # word vector size =300
    print "word2vec loaded!"
    print "num words already in word2vec: " + str(len(w2v))
    add_unknown_words(w2v, vocab)
    # update w2v to include words not in Google w2v
    W, word_idx_map = get_W(w2v)
    #Get word matrix. W[i] is the vector for word indexed by i. word_idx_map is
