...
def train_conv_net(datasets,
    ...
    datasets = [train, dev, test, train_fea, dev_fea, test_fea, train_text, dev_text, test_text]
    U = Word2Vec
    ...
    """    
    rng = np.random.RandomState(3435) # used to generate random initial weight of CNN
...
def make_idx_data(revs, fea, word_idx_map, max_l=51, k=300, filter_h=5):
    ...
    """
    # revs : list of datum
    # datum={"y": label, "text": sentence, "num_words": #words in sen, 
    #        "split": 0-train, 1-dev, 2-test}
    # fea = {'but_text': type: list; meaning: text after but, 
    #        'but_ind': type:list; 1 for yes, 0 for no but}
    train, dev, test = [], [], []
    train_text, dev_text, test_text = [], [], []
    train_fea, dev_fea, test_fea = {}, {}, {}
    fea['but'] = [] # resposible for store idx of word of but_text
    for k in fea.keys(): # 'but_text', 'but_ind', 'but'
        train_fea[k], dev_fea[k], test_fea[k] = [],[],[]
    for i,rev in enumerate(revs):
        sent = get_idx_from_sent(rev["text"], word_idx_map, max_l, k, filter_h)
        #len(sent) = max_l+2*filter_h-2, content of sent means the idx of word
        sent.append(rev["y"])
        fea['but'].append(get_idx_from_but_fea(fea['but_text'][i], fea['but_ind'][i]
                                               , word_idx_map, max_l, k, filter_h))
        if rev["split"]==0: # training data
            train.append(sent)
            for k,v in fea.iteritems(): # k = key, v = value
...
if __name__=="__main__":
    ...
    # datum={"y": label, "text": sentence, "num_words": #words in sen, 
    #        "split": 0-train, 1-dev, 2-test}
    # W[i] is the vector for word indexed by i. word_idx_map is 
    # a dict given a word output the idx to put into W.
    # W2 is random embedded.
    ...
    datasets = make_idx_data(revs, fea, word_idx_map, max_l=max_length, k=300, filter_h=5)
    #datasets = [train, dev, test, train_fea, dev_fea, test_fea, train_text, dev_text, test_text]
